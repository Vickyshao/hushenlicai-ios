...
- (void) setupBadgeViewWithString:(NSString *)string
{
    ...
    badgeSize.width = badgeSize.width < 15 ? 15 : badgeSize.width + 4;
    ...
    [badgeLabel setFrame:CGRectMake(self.bounds.size.width - 7.5 + horizontal, -(badgeSize.height / 2) - 7.5 + vertical, badgeSize.width,  badgeSize.width > 19 ? badgeSize.height : badgeSize.width)];
    [self setupBadgeStyle];
...
- (void) setupBadgeStyle
{
    ...
    badgeLabel.layer.cornerRadius = badgeLabel.bounds.size.width > 19 ? 7.5 : badgeLabel.bounds.size.width / 2;
}
